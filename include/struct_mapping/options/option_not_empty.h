...
public:
	...
	void check_option() const {
		static_assert(std::is_same_v<std::string, M> || detail::is_container_like_v<M>, "bad option (NotEmpty): option can only be applied to types: string, sequence container");
	}
